-- "Made to order" flag per product. Made-to-order items (sticker labels, name keychains)
-- need a conversation first, so the shop shows "Message us on Facebook" for them instead of
-- options + "Add to cart". Defaults to false (a normal cart product).
--
-- Also adds the shop's Messenger / Facebook link to app_settings ('' = not configured, which
-- hides the message button).
--
-- An earlier draft of this migration (applied to dev only) added allow_checkout /
-- allow_message and a check constraint; they're dropped here so dev and prod end up the same.

alter table products drop constraint if exists products_shop_action_check;
alter table products
  drop column if exists allow_checkout,
  drop column if exists allow_message;

alter table products add column if not exists made_to_order boolean not null default false;

alter table app_settings add column if not exists messenger_url text not null default '';

-- upsert_product: body unchanged from 20260924090000_add_products_show_in_shop.sql
-- apart from writing made_to_order.
...
declare
  ...
  prc jsonb;
begin
  insert into products (id, name, category, description, status, show_in_shop, made_to_order)
  values (
    v_product_id,
    payload->>'name',
...
-- 20260926090000_create_product_images.sql, plus made_to_order per row.
