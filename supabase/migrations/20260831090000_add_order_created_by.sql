-- Track who created an order via an id only (created_by uuid) — no
-- denormalized name column. Display names for both created_by and
-- status_updated_by are resolved live at read time (joined in list_orders,
-- looked up directly in getOrder), so a renamed user's history stays
-- accurate instead of showing a stale snapshot. This also drops the
-- existing status_updated_by_name column, moving it to the same live-lookup
-- approach.

alter table orders
  add column if not exists created_by uuid references users (id) on delete set null;

alter table orders drop column if exists status_updated_by_name;
...
begin
  insert into orders (
    ...
    created_by, status_updated_by, status_updated_at
  )
  values (
    v_order_id,
