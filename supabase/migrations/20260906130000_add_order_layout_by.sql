-- Track who did the layout work on an order, same treatment as created_by/
-- status_updated_by: an id only (layout_by uuid), display name resolved live
-- at read time (joined in list_orders, looked up directly in getOrder) rather
-- than stored as a denormalized snapshot.

alter table orders
  add column if not exists layout_by uuid references users (id) on delete set null;
