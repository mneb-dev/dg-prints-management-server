-- Add an optional layout fee to orders, alongside the existing
-- additional_fees column. Same treatment: stored as sent by the client,
-- no server-side recomputation of total.

alter table orders
  add column if not exists layout_fee numeric(10, 2) not null default 0;
