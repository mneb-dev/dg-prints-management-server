-- list_orders sorted order_number as text, so once numbers passed ORD-999 the
-- list ordered ORD-100, ORD-1000, ORD-1012, ORD-101, ... Sort by the numeric
-- suffix instead (via sort_num). Rows without a trailing number sort as null
-- and fall back to created_at. Otherwise identical to the definition in
      ...
      from (
        select * from (
          select
            ...
            case p_sort_by
              when 'order_number' then (substring(o.order_number from '(\d+)$'))::numeric
