-- Adds an order-level "description" field (distinct from the existing
-- order-level `notes` column, which is unrelated and unused, and from
-- `order_items.notes`, which is per-line-item text already wired into the
-- order form). Enforced at 20 characters in the API layer; no DB-level
-- length constraint, consistent with the rest of this schema.

alter table orders add column if not exists description text not null default '';

-- Re-issue upsert_order with `description` added alongside `notes` in the
-- insert column list, values, and on-conflict update clause.
--
...
--   "description": text, "channel": text, "additional_fees": number,
--   "shipping_address": jsonb|null, "payment_status": text,
--   "payment_method": text|null, "payment_down_payment": number,
